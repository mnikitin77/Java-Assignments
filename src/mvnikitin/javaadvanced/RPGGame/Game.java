package mvnikitin.javaadvanced.RPGGame;
...
class Game {
    ...
    private void init() {
        team1 = new Hero[]{new Warrior(250, "Тигрил", 50, 0)
                , new Assasin(150, "Акали", 70, 0)
                , new Doctor(120, "Жанна", 0, 60)};


        team2 = new Hero[]{new Warrior(290, "Минотавр", 60, 0)
                , new Assasin(160, "Джинкс", 90, 0)
                , new Doctor(110, "Зои", 0, 80)};
    }

    private boolean isTeamActive(Hero[] team) {
        boolean res = false;
        for (Hero h: team) {
            if (h.getHealth() >= 0) {
                res = true;
